import 'package:flutter/material.dart';
import 'package:hive_music_player/hive/model/all_songs/model.dart';
import 'package:hive_music_player/screens/home/screen_home.dart';
import 'package:hive_music_player/screens/now_playing/screen_now_playing.dart';
import 'package:hive_music_player/screens/now_playing/screen_now_playing_mini.dart';
import 'package:just_audio/just_audio.dart';
import 'package:on_audio_query/on_audio_query.dart';

final justAudioPlayerObject = AudioPlayer();
int miniIndex = 0;
final ValueNotifier<List<AudioModel>> miniSongList = ValueNotifier([]);

class MiniPlayer extends StatefulWidget {
  const MiniPlayer({super.key});

  @override
  State<MiniPlayer> createState() => _MiniPlayerState();
}

class _MiniPlayerState extends State<MiniPlayer> {
  final list = globalMiniList.value;

  @override
  Widget build(BuildContext context) {
    //------------------------------------------mini player bottom
    return globalMiniList.value.isNotEmpty
        ? Padding(
            padding: const EdgeInsets.only(bottom: 8, left: 8, right: 8),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(10),
              child: Container(
                color: Colors.transparent.withOpacity(0.5),
                height: 60,
                child: Row(
                  children: [
                    const SizedBox(
                      width: 10,
                    ),

                    ValueListenableBuilder(
                      valueListenable: miniPlayerIndex,
                      builder: (context, value, child) {
                        return QueryArtworkWidget(
                            artworkWidth: 40,
                            artworkHeight: 35,
                            artworkBorder: BorderRadius.circular(10),
                            nullArtworkWidget: ClipRRect(
                              borderRadius: BorderRadius.circular(10),
                              child: Image.asset(
                                'assets/song_tile_empty.png',
                                width: 40,
                                height: 35,
                                fit: BoxFit.cover,
                              ),
                            ),
                            id: list[miniPlayerIndex.value].id!,
                            type: ArtworkType.AUDIO);
                      },
                    ),
                    const SizedBox(
                      width: 5,
                    ),
                    Expanded(
                        child: ValueListenableBuilder(
                      valueListenable: globalMiniList,
                      builder: (context, globalList, child) {
                        if (globalList.isEmpty) {
                          return const SizedBox();
                        } else {
                          return GestureDetector(
                            onTap: () async {
                              Navigator.of(context).push(MaterialPageRoute(
                                builder: (context) {
                                  return ScreenNowPlaying1(
                                    passSongList: globalList,
                                  );
                                },
                              ));
                            },
                            child: ValueListenableBuilder(
                              valueListenable: miniPlayerIndex,
                              builder: (context, value, child) {
                                return Text(
                                  globalList[miniPlayerIndex.value].title!,
                                  overflow: TextOverflow.ellipsis,
                                  maxLines: 1,
                                  style: const TextStyle(
                                      fontSize: 16, color: Colors.white),
                                );
                              },
                            ),
                          );
                        }
                      },
                    )),

                    //------------------------------------seekbutton
                    GestureDetector(
                      onTap: () {
                        //------------------------------------------seek previous
                        if (justAudioPlayerObject.hasPrevious) {
                          justAudioPlayerObject.seekToPrevious();
                        }
                      },
                      child: const Icon(
                        Icons.skip_previous,
                        size: 30,
                        color: Colors.white,
                      ),
                    ),

                    //-----------------------------------------play or pause button
                    GestureDetector(
                      onTap: () async {
                        if (justAudioPlayerObject.playing) {
                          await justAudioPlayerObject.pause();
                          //await justAudioPlayerObject.stop();
                        } else {
                          if (justAudioPlayerObject.currentIndex != null) {
                            justAudioPlayerObject.play();
                          }
                        }
                      },
                      child: StreamBuilder<bool>(
                        stream: justAudioPlayerObject.playingStream,
                        builder: (context, snapshot) {
                          bool? playingState = snapshot.data;

                          if (playingState != null && playingState) {
                            return const CircleAvatar(
                              backgroundColor: Colors.black,
                              radius: 20,
                              child: Icon(
                                Icons.pause,
                                size: 25,
                              ),
                            );
                          }

                          return const CircleAvatar(
                            radius: 20,
                            backgroundColor: Colors.black,
                            child: Icon(
                              Icons.play_arrow,
                              size: 30,
                            ),
                          );
                        },
                      ),
                    ),

                    //---------------------------------------seek next function
                    GestureDetector(
                      onTap: () {
                        if (justAudioPlayerObject.hasNext) {
                          justAudioPlayerObject.seekToNext();
                        }
                      },
                      child: const Icon(
                        Icons.skip_next,
                        size: 30,
                        color: Colors.white,
                      ),
                    ),
                    const SizedBox(
                      width: 20,
                    ),
                    //------------------------------------------mini player close function

                    GestureDetector(
                        onTap: () {
                          miniPlayerStatusNotifier.value = false;
                          miniPlayerStatusNotifier.notifyListeners();

                          justAudioPlayerObject.pause();
                        },
                        child: const Icon(
                          Icons.close,
                          color: Colors.white,
                        )),
                    const SizedBox(
                      width: 5,
                    )
                  ],
                ),
              ),
            ),
          )
        : const SizedBox();
  }
}
